export default async function handler(req, res) {
  ...
  try {
    ...
    if (!accessToken || !workspaceHost) {
      console.log('[Knowledge Base List] Missing auth:', { hasToken: !!accessToken, hasHost: !!workspaceHost });
      return res.status(401).json({ error: 'Authentication required' });
    }

    console.log('[Knowledge Base List] Query params:', { scope, category, brand, fileType, isApproved, projectType, uploadedBy, searchTerm });
    ...
    console.log('[Knowledge Base List] Executing SQL query:', selectQuery);
